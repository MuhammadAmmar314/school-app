...
export default function Edit({ auth, teacher }) {
    const { data, setData, put, processing, errors } = useForm({ nama: teacher.user.name, nip: teacher.nip, email: teacher.user.email, password: '' });

    const submit = (e) => {
        e.preventDefault();
        put(route('admin.teacher.update', teacher));
    };

    return (
        <AuthenticatedLayout user={auth.user} header="Edit Guru">
